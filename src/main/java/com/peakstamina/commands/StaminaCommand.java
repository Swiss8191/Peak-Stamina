...
public class StaminaCommand {
        ...
        dispatcher.register(Commands.literal("stamina")
            ...
            .then(Commands.literal("reload")
                .executes(context -> {
                    ...
                    // Optional compat reload isolation
                    ...
                    context.getSource().sendSuccess(() -> Component.literal("§a[PeakStamina] Configs, compat, and weight caches successfully reloaded!"), true);
                    return 1;
                })
            )
        );
    ...
    private static void reloadParCool() {
        com.peakstamina.compat.parcool.ParCoolCompat.refreshCache();
    }

    private static void reloadWallJump() {
    ...
    private static int debugStamina(CommandSourceStack source, ServerPlayer target) {
        StaminaData cap = target.getData(StaminaCapability.STAMINA);
        ...
        java.util.function.Function<String, String> getCategory = (name) -> {
            ...
            if (name.contains("cost") || name.contains("usage")) return "Cost";
        ...
        for (String category : new String[]{"Core", "Cost", "Penalty", "Bonus"}) {
            List<net.neoforged.neoforge.registries.DeferredHolder<Attribute, ? extends Attribute>> entries = grouped.getOrDefault(category, List.of());
            ...
            entries.stream()
                ...
                .forEach(attrObj -> {
                    msg.append("§7").append(attrObj.getId().getPath()).append(": §f")
                       .append(String.format("%.2f", getAttr(target, (Holder<Attribute>) attrObj))).append("\n");
                });
        }
