...
public class ClientStaminaEvents {
    ...
    private static final ResourceLocation TEXTURE_SHEET = new ResourceLocation(peakStaminaMod.MODID, "textures/gui/peakstamina_texture_sheet.png");
    private static final ResourceLocation LAYOUT_JSON = new ResourceLocation(peakStaminaMod.MODID, "gui_layout.json");
    ...
    private static void loadDefaultLayout() {
        ...
        textureLayout.put("regen_pos_1", new int[]{26, 96, 4, 8});
        textureLayout.put("regen_pos_2", new int[]{26, 106, 10, 8});
        textureLayout.put("regen_pos_3", new int[]{26, 116, 16, 8});
        textureLayout.put("regen_neg_1", new int[]{26, 126, 4, 8});
        textureLayout.put("regen_neg_2", new int[]{26, 136, 10, 8});
        textureLayout.put("regen_neg_3", new int[]{26, 146, 16, 8});
    }
    ...
    private static void ensureLayoutLoaded() {
        ...
        try {
            ...
            System.err.println("[PeakStamina] Failed to load gui_layout.json, falling back to defaults. Error: " + e.getMessage());
        }
        layoutLoaded = true;
    }
    ...
    private static void drawTexturedQuadGradient(GuiGraphics gfx, int x, int y, int w, int h, int u, int v, int tw, int th, int texW, int texH, int colorTop, int colorBottom) {
        ...
        buffer.vertex(matrix, x, y, 0).uv(minU, minV).color(tR, tG, tB, tA).endVertex();
        buffer.vertex(matrix, x, y + h, 0).uv(minU, maxV).color(bR, bG, bB, bA).endVertex();
        buffer.vertex(matrix, x + w, y + h, 0).uv(maxU, maxV).color(bR, bG, bB, bA).endVertex();
        buffer.vertex(matrix, x + w, y, 0).uv(maxU, minV).color(tR, tG, tB, tA).endVertex();
    ...
    private static void renderStaminaHUD(GuiGraphics gfx) {
        ...
        mc.player.getCapability(StaminaCapability.INSTANCE).ifPresent(cap -> {
            ...
            float poisonTarget = Math.min(cap.poisonPenalty, baseMax);
            float weightTarget = Math.min(cap.weightPenalty, baseMax);

            displayedStamina += (cap.stamina - displayedStamina) * 0.2f;
            displayedBonusStamina += (cap.bonusStamina - displayedBonusStamina) * 0.2f;

            displayedPenalty += (fatigueTarget - displayedPenalty) * 0.1f;
            ...
            if (Math.abs(fatigueTarget - displayedPenalty) < 0.05f) displayedPenalty = fatigueTarget;
            ...
            if (cap.penaltyValues != null) {
                for (int i = 0; i < cap.penaltyValues.length; i++) {
                    ...
                    float targetVal = Math.min(cap.penaltyValues[i], baseMax);
        ...
        if (event.getScreen() instanceof net.minecraft.client.gui.screens.inventory.InventoryScreen invScreen) {
            ...
            if (mc.player != null) {
                mc.player.getCapability(StaminaCapability.INSTANCE).ifPresent(cap -> {
                    ...
                    AttributeInstance wMultAttr = mc.player.getAttribute(StaminaAttributes.WEIGHT_CALC_MULTIPLIER.get());
                    ...
                    AttributeInstance limitAttr = mc.player.getAttribute(StaminaAttributes.WEIGHT_LIMIT.get());
                    if (limitAttr != null) {
    ...
    public static String getFormattedWeight(double weight) {
        ...
        if (unit == com.peakstamina.config.StaminaConfig.Client.WeightUnit.kg) {
            multiplier = 0.453592;
            label = "kg";
        ...
        return String.format("%.1f %s", weight * multiplier, label);
    }
    ...
    public static String getFormattedWeightDual(double current, double max) {
        ...
        return String.format("%.1f / %.1f %s", current * multiplier, max * multiplier, label);
    }
}