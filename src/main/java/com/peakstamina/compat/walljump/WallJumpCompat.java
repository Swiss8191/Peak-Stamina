...
public class WallJumpCompat {
    ...
    public static void tick(Player player, StaminaData cap) {
        ...
        // // Print a debug line once per second to verify the drain is calculating
        // if (player.tickCount % 20 == 0) {
        //     System.out.println("[PEAK DEBUG - SERVER TICK] " + player.getName().getString() + " | Clinging: " + clinging + " | Speeding: " + speeding + " | Drain/tick: " + totalDrain);
        // }

        if (totalDrain > 0) {
            // Direct subtraction ensures it bypasses any filters in consumeStamina()
            cap.stamina -= (float) (totalDrain * usageMult);
            cap.staminaRegenDelay = StaminaConfig.COMMON.recoveryDelay.get(); // Prevent immediate regeneration
